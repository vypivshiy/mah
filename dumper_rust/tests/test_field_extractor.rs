...
fn test_dynamic_extract_fields_by_model_name() {
    ...
    assert!(
        ss_fields.len() > 50,
        "ServerSettings should have many fields"
    );
    for f in &ss_fields {
    ...
    assert_eq!(
        va_map.get("collage"),
        Some(&"std::optional<Api::OneMe::Types::VideoCollage>")
    );
    assert_eq!(va_map.get("paid"), Some(&"std::optional<bool>"));
    assert_eq!(
        va_map.get("previewData"),
        Some(&"std::optional<std::vector<std::byte>>")
    );
    assert_eq!(
        va_map.get("wave"),
        Some(&"std::optional<std::vector<std::byte>>")
