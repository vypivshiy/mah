...
use crate::type_parser::extract_template_args;
use anyhow::Result;
...
impl ClassHierarchy {
    ...
    pub fn is_base_of(&self, base: &str, model: &str) -> bool {
        if base == model {
            return false;
        }

        let mut stack = self.base_to_derived.get(base).cloned().unwrap_or_default();
        ...
        while let Some(candidate) = stack.pop() {
            ...
            if let Some(derived) = self.base_to_derived.get(&candidate) {
    ...
    pub fn bases_of(&self, model: &str) -> Vec<String> {
        let mut bases: Vec<String> = self
            .base_to_derived
            .keys()
            .filter(|base| self.is_base_of(base, model))
            .cloned()
            .collect();
        bases.sort();
        bases
...
pub struct SerializableMemberMetadata {
...
pub struct RttiEngine {
    ...
    pub smember_vtables: HashMap<u32, SerializableMemberMetadata>,
...
impl RttiEngine {
    pub fn build(pe: &PeImage) -> Result<Self> {
        ...
        for sec_name in &[".data", ".rdata"] {
            if let (Some(sec_data), Some(sec_info)) =
                (pe.section_data(sec_name), pe.section_by_name(sec_name))
            {
                let mut pos = 0;
                    ...
                    if b == b".?AV" || b == b".?AU" {
                        ...
                        if string_rva >= 16 {
                            ...
                            if let Some(mangled) = pe.read_cstring(string_rva) {
                                if mangled.len() >= 4
                                    && (mangled.starts_with(".?AV") || mangled.starts_with(".?AU"))
                                {
        ...
        if let (Some(rdata), Some(rdata_sec)) =
            (pe.section_data(".rdata"), pe.section_by_name(".rdata"))
        {
            let mut pos = 0;
                    ...
                    if let Some(col) = col_map.get(&target_rva) {
                        if let Some(td) = type_descriptors.get(&col.p_type_descriptor) {
                            ...
                            type_to_vtable
                                ...
                                .or_insert(vtable_rva);

                            if let Some(metadata) =
                                extract_smember_metadata(demangled, &td.mangled_name)
                            {
                                smember_vtables.insert(vtable_rva, metadata);
                            }
                        }
                    }
                }
...
pub fn extract_smember_metadata(
    demangled: &str,
    mangled: &str,
    ...
    if let Some(start) = demangled.find("SerializableMember<") {
        ...
        if start == 0 || !demangled[..start].ends_with('I') {
            if let Some(inner) =
                extract_template_body_after(demangled, start + "SerializableMember<".len())
            {
                let args = extract_template_args(inner);
                if let Some(first_arg) = args.first() {
                    let value_type = clean_demangled_rtti_name(first_arg);
                    if !value_type.contains("SerializedType")
                        && !value_type.contains("ISerializableMember")
                    {
                        let owner_type = args
                            .get(3)
                            .map(|owner| clean_demangled_rtti_name(owner))
                            .filter(|owner| !owner.is_empty());
                        return Some(SerializableMemberMetadata {
                            value_type,
                            owner_type,
                        });
                    }
    ...
    if let Some(pos) = mangled.find("?$SerializableMember@") {
        ...
        if pos == 0 || !mangled[..pos].ends_with('I') {
            ...
            if let Ok(dem) =
                msvc_demangler::demangle(&r0_symbol, msvc_demangler::DemangleFlags::COMPLETE)
            {
...
pub fn extract_smember_type(demangled: &str, mangled: &str) -> Option<String> {
    extract_smember_metadata(demangled, mangled).map(|metadata| metadata.value_type)
}

fn extract_template_body_after(s: &str, offset: usize) -> Option<&str> {
    let inner = &s[offset..];
    let mut depth = 0;
