...
use crate::rtti::{RttiEngine, SerializableMemberMetadata};
...
impl FieldCandidate {
    pub fn into_field(self) -> ExtractedField {
        ExtractedField {
            name: self.name,
    ...
    while decoder.can_decode() {
            ...
            && insn.memory_base() == Register::RIP
        {
            ...
            if target_va >= pe.image_base {
                ...
                if let Some(member) = rtti.smember_vtables.get(&target_rva) {
                    if !member.value_type.contains("SerializedType")
                        && !member.value_type.contains("ISerializableMember")
                    {
                        return true;
                    }
                }
...
impl VtableXrefIndex {
    pub fn build(pe: &PeImage, rtti: &RttiEngine) -> Self {
        ...
        if let (Some(text_sec), Some(text_data)) =
            (pe.section_by_name(".text"), pe.section_data(".text"))
        {
            let mut decoder = Decoder::with_ip(
                64,
                text_data,
        ...
        for &func_rva in eval_funcs {
            if let Ok(fields) = extractor.extract_from_func(func_rva) {
                ...
                if unique_count > best_unique_count {
                    ...
                    let curr_len = extractor
                        .pe
                        .find_function(func_rva)
                        .map(|f| f.len())
                        .unwrap_or(u32::MAX);
                    let best_len = extractor
                        .pe
                        .find_function(best_func)
                        .map(|f| f.len())
                        .unwrap_or(u32::MAX);
...
struct PendingField {
    name: String,
    insn_idx: usize,
    type_metadata: Option<SerializableMemberMetadata>,
...
impl<'a> FieldExtractor<'a> {
    ...
    pub fn extract_from_func(&self, func_rva: u32) -> Result<Vec<ExtractedField>> {
        Ok(self
            ...
            .into_iter()
            .map(FieldCandidate::into_field)
            .collect())
    }

    pub fn extract_candidates_from_func(&self, func_rva: u32) -> Result<Vec<FieldCandidate>> {
        ...
        let mut helper_cache: HashMap<u32, Option<SerializableMemberMetadata>> = HashMap::new();
        ...
        let commit_field = |pf: PendingField, fields: &mut Vec<FieldCandidate>| {
            if let Some(metadata) = pf.type_metadata {
                let field_t = decompose_type(&metadata.value_type);
                ...
                if !fields.iter().any(|f| f.name == pf.name) {
                    fields.push(FieldCandidate {
                        ...
                        name_source: "string_literal".to_string(),
                        type_source: "serializable_member".to_string(),
            ...
            match insn.mnemonic() {
                Mnemonic::Lea => {
                    ...
                    if insn.op1_kind() == OpKind::Memory && insn.memory_base() == Register::RIP {
                        ...
                        if target_va >= self.pe.image_base {
                            ...
                            if let Some(member) = self.rtti.smember_vtables.get(&target_rva) {
                                if !member.value_type.contains("SerializedType")
                                    && !member.value_type.contains("ISerializableMember")
                                {
                                    reg_state
                                        .insert(dest_reg, RegVal::MemberVtable(member.clone()));
                                    if let Some(ref mut pf) = pending_field {
                                        if insn_idx.saturating_sub(pf.insn_idx) <= 35 {
                                            pf.type_metadata = Some(member.clone());
                ...
                Mnemonic::Mov => {
                        ...
                        && insn.memory_base() != Register::None
                    {
                            ...
                            || insn.op1_kind() == OpKind::Immediate8
                        {
                            ...
                            if imm == 1 || imm == 2 {
                                if let Some(ref mut pf) = pending_field {
                                    if pf.type_metadata.is_some()
                                        && insn_idx.saturating_sub(pf.type_insn_idx.unwrap_or(0))
                                            <= 10
                                    {
                            ...
                            if insn.op1_register().is_gpr64() {
                                ...
                                if let Some(RegVal::MemberVtable(metadata)) =
                                    reg_state.get(&src_reg)
                                {
                                    if let Some(ref mut pf) = pending_field {
                                        if insn_idx.saturating_sub(pf.insn_idx) <= 35 {
                                            pf.type_metadata = Some(metadata.clone());
                                            pf.type_insn_idx = Some(insn_idx);
                                        }
                                    }
                                }
                        ...
                        } else if insn.op1_kind() == OpKind::Immediate32
                            || insn.op1_kind() == OpKind::Immediate8
                        {
                            let imm = insn.immediate32();
                ...
                Mnemonic::Xor | Mnemonic::Sub => {
                    if insn.op0_kind() == OpKind::Register {
                        let dest_reg = insn.op0_register().full_register();
                        if insn.op1_kind() == OpKind::Register
                            && dest_reg == insn.op1_register().full_register()
                        {
                            reg_state.insert(dest_reg, RegVal::Imm);
                        } else {
                            reg_state.remove(&dest_reg);
                ...
                Mnemonic::Call => {
                    if let Some(ref mut pf) = pending_field {
                        ...
                        if pf.type_metadata.is_none() && insn_idx.saturating_sub(pf.insn_idx) <= 35
                        {
                            let target_va = insn.near_branch64();
                            if target_va >= self.pe.image_base {
                                let target_rva = (target_va - self.pe.image_base) as u32;
                                if target_rva != 0x35477 {
                                    if let Some(metadata) =
                                        self.infer_helper_metadata(target_rva, &mut helper_cache, 0)
                                    {
                                        pf.type_metadata = Some(metadata);
                                        pf.type_insn_idx = Some(insn_idx);
                                    }
                                }
                            }
                    ...
                    for v_reg in &X64_VOLATILE_REGISTERS {
    ...
    fn infer_helper_metadata(
        &self,
        target_rva: u32,
        cache: &mut HashMap<u32, Option<SerializableMemberMetadata>>,
        depth: usize,
    ) -> Option<SerializableMemberMetadata> {
        if depth > 4 {
            return None;
        }
        ...
        while decoder.can_decode() && insn_count < 50 {
                ...
                && insn.memory_base() == Register::RIP
            {
                ...
                if target_va >= self.pe.image_base {
                    ...
                    if let Some(member) = self.rtti.smember_vtables.get(&rva) {
                        if !member.value_type.contains("SerializedType")
                            && !member.value_type.contains("ISerializableMember")
                        {
                            last_metadata = Some(member.clone());
                            break;
                        }
                    }
                }
                ...
                if insn.op0_kind() == OpKind::NearBranch64 {
                    ...
                    if jmp_va >= self.pe.image_base {
                        ...
                        if let Some(metadata) =
                            self.infer_helper_metadata(jmp_rva, cache, depth + 1)
                        {
                ...
                if insn.op0_kind() == OpKind::NearBranch64 {
                    ...
                    if call_va >= self.pe.image_base {
                        ...
                        if let Some(metadata) =
                            self.infer_helper_metadata(call_rva, cache, depth + 1)
                        {
                            last_metadata = Some(metadata);
                            break;
                        }
        ...
        cache.insert(target_rva, last_metadata.clone());
        last_metadata
    }
}
...
enum RegVal {
    StringLiteral,
    MemberVtable(SerializableMemberMetadata),
    Imm,
}
