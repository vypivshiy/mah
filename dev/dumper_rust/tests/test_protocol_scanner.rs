...
fn test_scan_packets_and_events_invariants() {
    ...
    // Packets invariants
    assert!(!scanner.packets.is_empty(), "Packets list should not be empty");
    let mut packet_opcodes = HashSet::new();
    for p in &scanner.packets {
        assert!(p.opcode > 0);
        assert!(packet_opcodes.insert(p.opcode), "Duplicate opcode: {}", p.opcode);
        assert!(!p.request_full_name.is_empty());
        assert!(!p.response_full_name.is_empty());
    }

    // Events invariants
    assert!(!scanner.events.is_empty(), "Events list should not be empty");
    let mut event_opcodes = HashSet::new();
    for e in &scanner.events {
        assert!(e.opcode > 0);
        assert!(event_opcodes.insert(e.opcode), "Duplicate event opcode: {}", e.opcode);
    }
