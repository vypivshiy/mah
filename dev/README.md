...
dumper_rust/ - автономный standalone-дампер на Rust. Работает быстро: полный дамп за **~300 мс** (против десятков секунд в IDA и минут в Binary Ninja). Напрямую парсит MSVC RTTI из x64 PE `core.dll`, восстанавливает полиморфные модели и декодирует через `iced-x86`. Формирует канонический формат дампа (с декомпозицией типов как в binja), с которого генерируются клиенты.
