...
IDA_dumper.py - скрипт для IDA pro 7.1+ для дампа структур пакетов для последующей генерации SDK. требуется hex-rays декомпилятор, на free версии не будет работать.

Извлечение:

1. скачать windows client https://download.max.ru/#desktop
2. распаковать или установить. Требуется `*core.dll`, загрузить его в IDA pro
3. подождать сканирование
4. загрузить скрипт dumper/run.py,  ждать когда сохранит дамп 
5. дамп сохранится в директории откуда загружали dll в IDA-PRO, не в папке со скриптом
