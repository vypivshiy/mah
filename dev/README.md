...
dumper_rust/ - автономный высокопроизводительный дампер на Rust без внешних дизассемблеров и лицензий. Анализирует x64 PE `core.dll`, парсит MSVC RTTI, восстанавливает граф наследования полиморфных моделей и декодирует инструкции через `iced-x86`. Выполняет полный дамп за **~300 мс** (против десятков секунд в IDA и минут в Binary Ninja). Поддерживает два формата вывода (`--format binja` с богатой декомпозицией типов и `--format ida` для кодогенераторов SDK).
...
Структурированные дампы (`packets_rust.json` / `packets_binja.json` / `packets.json`) предназначены для кодогенерации готового кода структур данных в соответствии с описанием типов:

```
+--------------------+     +-----------------------------+     +-------------+
| packets_rust.json  |---->| codegen                     |---->| *.py / *.go | <--- любые расширения
+--------------------+     +-----------------------------+     +-------------+
```
