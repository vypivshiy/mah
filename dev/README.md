...
Извлечение:
1. скачать windows client https://download.max.ru/#desktop
2. Вы можете установить мессенджер и вручную найти `core.dll` файл или воспользуйтесь скриптами для его извлечения:

Нужно положить установочник `MAX.msi` в каталог. Требуется установленный архиватор 7zip в системе.
...
3. загрузить dll в ida-pro
4. подождать полное сканирование проекта (в левом нижнем углу должен быть label `AU: idle | Down` )
5. загрузить скрипт dumper/run.py, ждать когда сохранит дамп 
6. дамп сохранится в директории откуда загружали dll в IDA-PRO, не в папке со скриптом
